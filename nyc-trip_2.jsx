...
const DAYS = [
  {
    ...
    events: [
      { time: "11:10", end: "12:00", title: "JFK 공항 도착", type: "transport", location: "John F. Kennedy International Airport", address: "Queens, NY 11430",
        ...
        photo: "https://images.unsplash.com/photo-1570710891163-6d3b5c47248b?w=600&h=300&fit=crop",
      ...
      { time: "19:00", end: "20:30", title: "미드타운 캐주얼 디너", type: "food", location: "미드타운 (타임스퀘어 주변)", address: "W 40~50th St, Manhattan",
        description: "첫날은 시차 적응 겸 가볍게!\n\n🍔 캐주얼 추천\n• Shake Shack (Theater District) - 버거+쉐이크\n• Five Guys (Times Sq) - 뉴욕 정통 버거\n• Joe's Pizza (7th Ave) - 뉴요커 슬라이스 피자\n• The Halal Guys (53rd & 6th) - 뉴욕 길거리 푸드 클래식\n• Los Tacos No.1 (Times Sq점) - 가성비 타코\n\n💰 1인 $15~25",
        photo: "https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=600&h=300&fit=crop",
        tip: "Halal Guys 또는 Joe's Pizza가 진짜 뉴요커 첫끼 느낌!",
        transit: [
          { mode: "walk", icon: "🚶", label: "타임스퀘어에서 도보", detail: "타임스퀘어 주변 모두 도보 5~10분 거리" }
        ]},
      { time: "21:00", title: "숙소 복귀", type: "transport", location: "미드타운 → 숙소", address: "페리 또는 Uber",
        description: "페리(W. 39th St) 또는 Uber로 편하게 복귀.\n첫날은 일찍 쉬고 내일을 위해 컨디션 관리!",
        tip: "페리 막차 시간 미리 확인! (보통 평일 ~21:00)",
        transit: [
          { mode: "ferry", icon: "⛴️", label: "W. 39th St → Port Imperial", detail: "NY Waterway · 8분 · $9 · 막차 시간 체크" },
          { mode: "car", icon: "🚗", label: "또는 Uber/Lyft", detail: "미드타운 → 숙소 · 약 15~20분 · $25~35 (1인 $8~12)" }
        ]}
    ]
  },
  {
    ...
    events: [
      ...
      { time: "15:30", end: "17:30", title: "DUMBO & Brooklyn Bridge Park", type: "sightseeing", location: "DUMBO, Brooklyn", address: "Washington St & Water St, Brooklyn, NY 11201",
        ...
        photo: "https://images.unsplash.com/photo-1560618537-20dbd5981c72?w=600&h=300&fit=crop",
      ...
      { time: "18:30", end: "20:30", title: "St. Anselm 스테이크 디너", type: "highlight", location: "St. Anselm", address: "355 Metropolitan Ave, Brooklyn, NY 11211 (Williamsburg)",
        ...
        photo: "https://images.unsplash.com/photo-1544025162-d76694265947?w=600&h=300&fit=crop",
  ...
  },
  {
    ...
    events: [
      ...
      { time: "10:30", end: "12:00", title: "Balthazar 브런치", type: "highlight", location: "Balthazar", address: "80 Spring St, New York, NY 10012 (SoHo)",
        ...
        photo: "https://images.unsplash.com/photo-1504754524776-8f4f37790ca0?w=600&h=300&fit=crop",
      ...
      { time: "17:30", end: "19:00", title: "브라이언파크 맥주 한잔", type: "highlight", location: "The Porch at Bryant Park", address: "41 W 40th St, New York, NY 10018",
        ...
        photo: "https://images.unsplash.com/photo-1555854877-bab0e564b8d5?w=600&h=300&fit=crop",
  ...
  },
  {
    ...
    events: [
      ...
      { time: "14:30", end: "17:30", title: "메트로폴리탄 미술관 (The Met)", type: "highlight", location: "The Metropolitan Museum of Art", address: "1000 5th Ave, New York, NY 10028",
        ...
        photo: "https://images.unsplash.com/photo-1575505586569-646b2ca898fc?w=600&h=300&fit=crop",
  ...
  },
  {
    ...
    events: [
      ...
      { time: "10:30", end: "12:00", title: "Chelsea Market 브런치", type: "food", location: "Chelsea Market", address: "75 9th Ave, NY 10011",
        ...
        photo: "https://images.unsplash.com/photo-1555992336-fb0d29498b13?w=600&h=300&fit=crop",
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily:"'Pretendard',-apple-system,sans-serif", background:"#fff", color:"#333", minHeight:"100vh", maxWidth:480, margin:"0 auto" }}>
      ...
      <header style={{ padding:"48px 28px 0" }}>
        ...
        {/* 실시간 뉴욕 날씨 */}
        <div style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 16px", borderRadius:14, background:`linear-gradient(135deg, ${d.accent}10, ${d.accent}05)`, border:`1px solid ${d.accent}20` }}>
          {weather.loading ? (
      ...
      <nav style={{ display:"flex", gap:0, padding:"24px 28px 18px", overflowX:"auto", scrollbarWidth:"none" }}>
        {DAYS.map((dd,i)=>{const on=day===i;return <button key={i} onClick={()=>{setDay(i);setOpen(null);setPanel(null)}} style={{
          ...
          padding:"11px 18px", borderRadius:14, minWidth:64, cursor:"pointer", transition:"all 0.3s", border:"none",
          background:on?dd.accent:"transparent", color:on?"#fff":"#aaa",
        }}>
          <span style={{ fontSize:21, filter:on?"brightness(10)":"none" }}>{dd.emoji}</span>
          <span style={{ fontSize:13, fontWeight:on?700:500, whiteSpace:"nowrap" }}>{dd.date.replace("5월 ","")}</span>
          <span style={{ fontSize:11, opacity:0.7 }}>{dd.day}</span>
        </button>})}
      </nav>

      <div style={{ display:"flex", gap:10, padding:"0 28px 16px" }}>
        {[
          ...
          return (
            <button key={b.k} onClick={()=>setPanel(on?null:b.k)} style={{
              flex:1, padding:"11px 14px", borderRadius:14, cursor:"pointer", transition:"all 0.25s",
              ...
              <span style={{ flex:1, fontSize:16, fontWeight:700, color: on ? d.accent : "#333", lineHeight:1, whiteSpace:"nowrap" }}>{b.l}</span>
      ...
      {panel==="transport"&&<div style={{ padding:"0 28px 20px", animation:"fin 0.3s" }}>
        {Object.values(TRANSPORT).map((s,si)=><div key={si} style={{ marginBottom:16 }}>
          ...
          {s.routes.map((r,ri)=><div key={ri} style={{ background:"#fafaf8", borderRadius:14, padding:"16px 18px", marginBottom:8, border:"1px solid #f0efe8" }}>
      ...
      {panel==="checklist"&&<div style={{ padding:"0 28px 20px", animation:"fin 0.3s" }}>
        {/* 항목 추가 입력창 (상단) */}
        <div style={{ display:"flex", gap:6, marginBottom:12 }}>
          <input
            ...
            style={{
              flex:1, padding:"12px 14px", borderRadius:12, fontSize:16,
          ...
          <button onClick={addTodo} disabled={!todoInput.trim()} style={{
            padding:"0 18px", borderRadius:12, fontSize:16, fontWeight:700,
        ...
        {/* 진행률 */}
        {allTodos.length > 0 && (
          <div style={{ marginBottom:10, padding:"10px 14px", borderRadius:12, background:`${d.accent}08`, display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ flex:1 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
          {allTodos.length === 0 && (
            <div style={{ padding:"32px 16px", textAlign:"center", color:"#bbb", fontSize:16, background:"#fafaf8", borderRadius:14, border:"1px dashed #e7e7e3" }}>
          ...
          {allTodos.map((c)=>{
            ...
            return (
              <div key={c.id} onClick={()=>toggleTodo(c.id)} style={{
                ...
                borderRadius:14, border:`1px solid ${done?"#e8e8e4":"#f0efe8"}`,
                cursor:"pointer", transition:"all 0.15s",
              }}>
                <div style={{
                  width:34, height:34, borderRadius:10, flexShrink:0, fontSize:20,
        ...
        {todoState.removed?.length > 0 && (
          <button onClick={restoreDefaults} style={{
            marginTop:10, width:"100%", padding:"9px 0", borderRadius:10, fontSize:14, fontWeight:600,
            border:"1px dashed #ddd", background:"transparent", color:"#888", cursor:"pointer",
          }}>↺ 삭제한 기본 항목 복원 ({todoState.removed.length}개)</button>
        )}
      </div>}

      <div style={{ padding:"4px 28px 2px", display:"flex", alignItems:"baseline", gap:10 }}>
      ...
      <div style={{ padding:"10px 18px 130px 28px" }}>
        {d.events.map((ev,i)=>{
          const isO=open===`${day}-${i}`, isH=ev.type==="highlight", col=TM[ev.type]||TM.sightseeing;
          return <div key={`${day}-${i}`} style={{ animation:`fin 0.35s ease ${i*0.03}s both`, marginBottom:2 }}>
            <button onClick={()=>setOpen(isO?null:`${day}-${i}`)} style={{
              ...
              padding:"14px 14px 14px 0", borderRadius:16, transition:"all 0.2s",
              ...
              <div style={{ display:"flex", flexDirection:"column", alignItems:"center", paddingTop:6, width:12, flexShrink:0 }}>
                <div style={{ width:6, height:6, borderRadius:"50%", background:col+"44" }}/>
                {i<d.events.length-1&&<div style={{ width:1.5, flex:1, minHeight:8, background:"#f0f0ec", marginTop:4 }}/>}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                ...
                {isO&&<div style={{ animation:"fin 0.25s", marginTop:14 }} onClick={e=>e.stopPropagation()}>
                  {ev.photo&&<div style={{ borderRadius:14, overflow:"hidden", marginBottom:14, height:150, background:`linear-gradient(135deg, ${d.accent}15, ${d.accent}30)` }}>
                    <img src={ev.photo} alt="" loading="lazy" referrerPolicy="no-referrer" style={{ width:"100%", height:"100%", objectFit:"cover", display:"block" }} onError={e=>{e.currentTarget.parentElement.style.display="none"}}/>
                  </div>}
                  ...
                  <div style={{ display:"flex", gap:6, marginBottom:12 }}>
                    <a href={mapsUrl(ev.address, ev.location)} target="_blank" rel="noopener noreferrer" style={{
                      flex:1, padding:"9px 10px", borderRadius:10, fontSize:14, fontWeight:600,
                      background:"#fff", border:`1px solid ${d.accent}33`, color:d.accent,
                      textAlign:"center", textDecoration:"none", display:"flex", alignItems:"center", justifyContent:"center", gap:5,
                    }}>📍 지도에서 보기</a>
                  ...
                  {ev.transit && ev.transit.length > 0 && (
                    <div style={{ marginTop:14, background:"#f8f8f6", borderRadius:12, padding:"14px 16px", border:"1px solid #f0efe8" }}>
                  ...
                  {ev.tip&&<div style={{ marginTop:12, padding:"11px 14px", background:`${d.accent}08`, borderRadius:12, fontSize:15, color:d.accent, lineHeight:1.6, display:"flex", gap:6, fontWeight:500 }}>
                  ...
                  {(() => {
                    ...
                    return (
                      <div style={{ marginTop:12 }}>
                        {!isEdit && has && (
                          <div style={{ padding:"12px 14px", background:"#fffbe9", border:"1px solid #f3e7b8", borderRadius:12, position:"relative" }}>
                        ...
                        {isEdit && (
                          <div style={{ padding:"12px 14px", background:"#fffbe9", border:"1px solid #f3e7b8", borderRadius:12 }}>
                            ...
                            <textarea
                              ...
                              style={{
                                ...
                                border:"1px solid #e7d99a", borderRadius:8, background:"#fff", outline:"none",
                              }}
                            />
                            <div style={{ display:"flex", gap:6, marginTop:10, alignItems:"center" }}>
                              {has && (
                                <button onClick={()=>deleteMemo(k)} style={{ padding:"7px 12px", borderRadius:8, border:"1px solid #f0d3d3", background:"#fff", color:"#c44", fontSize:15, fontWeight:600, cursor:"pointer" }}>삭제</button>
                              )}
                              <span style={{ flex:1 }} />
                              <button onClick={cancelEdit} style={{ padding:"7px 14px", borderRadius:8, border:"1px solid #ddd", background:"#fff", color:"#666", fontSize:15, fontWeight:600, cursor:"pointer" }}>취소</button>
                              <button onClick={()=>saveMemo(k)} style={{ padding:"7px 14px", borderRadius:8, border:"none", background:d.accent, color:"#fff", fontSize:15, fontWeight:700, cursor:"pointer" }}>저장</button>
                            </div>
                          </div>
                        )}
                      </div>
      ...
      <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:480, background:"linear-gradient(transparent, #fff 40%)", padding:"40px 0 22px", display:"flex", justifyContent:"center", gap:6, pointerEvents:"none" }}>
        {DAYS.map((dd,i)=><button key={i} onClick={()=>{setDay(i);setOpen(null);setPanel(null)}} style={{
          width:day===i?24:6, height:6, borderRadius:3, border:"none",
